...
export const Hero = () => {
    ...
    return <div className="relative w-full bg-background flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="w-full z-0 absolute top-0 left-0">
            <img src={`${TMDB_IMAGE_BASE_URL}/${BACKDROP_SIZE}${nowPlaying[0]?.backdrop_path}`} alt="Hero" className="w-full h-auto object-cover" />
        </div>
        {/* Show Info */}
        ...
        <div className="z-10 p-8 text-white flex flex-col items-start">
            ...
            <p className="text-xl mb-2 flex items-center"><span><Star className="mx-1" /></span>{nowPlaying[0]?.vote_average.toFixed(2)}/10 ({nowPlaying[0]?.vote_count}) {nowPlaying[0]?.runtime} </p> 
